...
`camifit` uses a supervised Codex pair as the base workflow:
...
scripts/run_codex_pair_cycle.sh --seed-role-sessions
```

Use `audit_autonomous_workflow.sh` before starting, after interruptions, and before claiming a milestone.

The pair runner persists one Executor Codex thread and one Reviewer Codex thread in `.codex-role-sessions/` and resumes them on later cycles. Do not reset those markers unless the role thread is intentionally obsolete or cannot be resumed.
