...
usage() {
  cat <<'EOF'
Usage:
  scripts/start_codex_goal_loop.sh [options] [-- runner options]
...
Additional arguments after -- are passed to scripts/run_codex_pair_cycle.sh.
EOF
}
...
if [ "${#EXTRA_ARGS[@]}" -gt 0 ]; then
  nohup bash "$ROOT/scripts/run_codex_pair_cycle.sh" \
    --loop \
    --root "$ROOT" \
    --interval "$INTERVAL" \
