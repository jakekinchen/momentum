...
usage() {
  cat <<'EOF'
Usage:
  scripts/run_codex_pair_cycle.sh --once [options]
  scripts/run_codex_pair_cycle.sh --loop [options]
  scripts/run_codex_pair_cycle.sh --dry-run [options]
...
if [ ! -f GOAL.md ]; then
  printf 'GOAL.md missing. Fill GOAL.md before starting the loop.\n' >&2
  exit 1
fi

if [ ! -f executor-reviewer-pair-programming.md ]; then
  printf 'executor-reviewer-pair-programming.md missing.\n' >&2
  exit 1
fi
...
repo_slug="$(printf '%s' "$(basename "$ROOT")" | tr -cs 'a-zA-Z0-9._-' '-')"
...
has_stop_sentinel() {
  grep -q '^[[:space:]]*<stop-orchestrator/>[[:space:]]*$' GOAL.md 2>/dev/null
}
...
run_role() {
  ...
  printf '\n== Running %s ==\n' "$role"
  printf 'log: %s\n' "$json_log"
  if ! codex "${codex_args[@]}" < "$prompt_file" > "$json_log" 2>&1; then
    printf '%s role failed. See %s\n' "$role" "$json_log" >&2
    node scripts/record_latest_codex_session_id.mjs "$role" "$json_log" "$last_msg" || true
    return 1
  fi
  node scripts/record_latest_codex_session_id.mjs "$role" "$json_log" "$last_msg"
...
write_executor_prompt() {
  out="$1"
  cat > "$out" <<'EOF'
You are the Executor in the FitGraph repo-local autonomous workflow.
...
4. Prove reachability from a real command, test, or demo path.
...
FitGraph product constraints:
- Deterministic graph traversal decides eligibility and safety.
- LLMs may parse and verbalize, but never decide safety.
- Unknown ontology or clinical codes must remain explicitly unverified.
- Do not use vector retrieval for safety enforcement.
...
write_reviewer_prompt() {
  out="$1"
  cat > "$out" <<'EOF'
You are the Reviewer / Planner in the FitGraph repo-local autonomous workflow.
...
Decision semantics:
- CONTINUE means the latest slice is valid and the next slice is clear.
- NUDGE means a tactical correction is needed before more work.
- REDIRECT means the active plan or docs are stale.
- STOP means mission complete, unsafe to continue, or waiting on human input.
- ESCALATE means Manager or human input is required.
...
while [ "$cycle" -le "$MAX_CYCLES" ]; do
  ...
  prompt_dir="$(mktemp -d -t fitgraph-workflow-prompts-XXXXXX)"
