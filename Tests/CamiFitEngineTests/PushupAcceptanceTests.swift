...
final class PushupAcceptanceTests: XCTestCase {
    func testBodyweightPushupPresetLoadsAndAcceptanceFixturesPass() throws {
        let program = try ProgramLoader.load(from: Self.presetURL)
        XCTAssertEqual(program.id, "bodyweight_pushup")

        let clean = try Self.loadPoseFixture("synthetic_pushup_clean_trace.json")
        let shallow = try Self.loadPoseFixture("synthetic_pushup_shallow_trace.json")
    ...
    private static func assertAcceptance(
        ...
        print("pushup-acceptance-trace-\(name)\n\(Self.rowsContaining(formatted, " | true | "))")
    }
    ...
    private static var presetURL: URL {
        packageRoot.appendingPathComponent("Presets/bodyweight_pushup.json")
    }
