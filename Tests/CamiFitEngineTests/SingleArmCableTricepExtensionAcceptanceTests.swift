...
final class SingleArmCableTricepExtensionAcceptanceTests: XCTestCase {
    func testSingleArmCableTricepExtensionPresetLoadsAndAcceptanceTracesPass() throws {
        let program = try ProgramLoader.load(from: Self.presetURL)
        XCTAssertEqual(program.id, "single_arm_cable_tricep_extension")
    ...
    private static func assertAcceptance(
        ...
        print(
            [
                "single-arm-cable-tricep-extension-acceptance",
                "case=\(name)",
                "frames=\(frames.count)",
        ...
        print("single-arm-cable-tricep-extension-acceptance-trace-\(name)\n\(Self.rowsContaining(formatted, "elbow_angle"))")
    }
    ...
    private static func frame(timestampMS: Int64, factor: Double) -> PoseFrame {
        let primary: [String: PoseLandmark] = [
            ...
            "elbow": point(0.500, 0.480, 0.03),
            "wrist": point(mix(0.590, 0.510, factor), mix(0.470, 0.720, factor), 0.08),
    ...
    private static var presetURL: URL {
        packageRoot.appendingPathComponent("Presets/single_arm_cable_tricep_extension.json")
    }
