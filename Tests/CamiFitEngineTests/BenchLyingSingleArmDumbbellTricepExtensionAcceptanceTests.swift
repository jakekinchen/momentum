...
final class BenchLyingSingleArmDumbbellTricepExtensionAcceptanceTests: XCTestCase {
    func testBenchLyingSingleArmDumbbellTricepExtensionPresetLoadsAndAcceptanceTracesPass() throws {
        let program = try ProgramLoader.load(from: Self.presetURL)
        XCTAssertEqual(program.id, "bench_lying_single_arm_dumbbell_tricep_extension")
    ...
    private static func assertAcceptance(
        ...
        print(
            [
                "bench-lying-single-arm-dumbbell-tricep-extension-acceptance",
                "case=\(name)",
                "frames=\(frames.count)",
        ...
        print("bench-lying-single-arm-dumbbell-tricep-extension-acceptance-trace-\(name)\n\(Self.rowsContaining(formatted, "elbow_angle"))")
    }
    ...
    private static func frame(timestampMS: Int64, factor: Double) -> PoseFrame {
        let primary: [String: PoseLandmark] = [
            "nose": point(0.300, 0.540, -0.03),
            "shoulder": point(0.380, 0.550, 0),
            "elbow": point(0.540, 0.380, 0.03),
            "wrist": point(mix(0.700, 0.430, factor), mix(0.210, 0.310, factor), 0.08),
            "hip": point(0.700, 0.600, 0),
            "knee": point(0.820, 0.640, 0.02),
            "ankle": point(0.920, 0.680, 0.05)
        ]
        ...
        for (joint, value) in primary {
            ...
            if joint != "nose" {
                landmarks["left.\(joint)"] = point(value.x, value.y, value.z + 0.10)
                landmarks["right.\(joint)"] = point(value.x + 0.06, value.y, value.z - 0.10)
            }
        }
    ...
    private static var presetURL: URL {
        packageRoot.appendingPathComponent("Presets/bench_lying_single_arm_dumbbell_tricep_extension.json")
    }
